...
function makeLoader(callback) {
    ...
    return function(source, inputSourceMap) {
        ...
        const cb = this.async();
        loader.call(this, source, inputSourceMap, overrides).then((args)=>cb(null, ...args)
        , (err)=>cb(err)
        );
    };
}
async function loader(source, inputSourceMap, overrides) {
    ...
    const loaderSpan = (0, _trace).trace('babel-loader', (ref = this.currentTraceSpan) === null || ref === void 0 ? void 0 : ref.id);
    return loaderSpan.traceAsyncFn(async ()=>{
        const filename = this.resourcePath;
        loaderSpan.setAttribute('filename', filename);
        let loaderOptions = _loaderUtils.default.getOptions(this) || {
        };
        let customOptions;
        if (overrides && overrides.customOptions) {
            const customoptionsSpan = (0, _trace).trace('loader-overrides-customoptions');
            const result = await customoptionsSpan.traceAsyncFn(async ()=>await overrides.customOptions.call(this, loaderOptions, {
                    source,
                    map: inputSourceMap
                })
            );
        ...
        const programmaticOptions = Object.assign({
        }, loaderOptions, {
            filename,
        ...
        const partialConfigSpan = (0, _trace).trace('babel-load-partial-config-async');
        ...
        if (config) {
            ...
            if (overrides && overrides.config) {
                const overridesSpan = (0, _trace).trace('loader-overrides-config');
                options = await overridesSpan.traceAsyncFn(async ()=>await overrides.config.call(this, config, {
                        source,
                        map: inputSourceMap,
                        customOptions
                    })
            ...
            if (cacheDirectory) {
                ...
                const transformSpan = (0, _trace).trace('transform');
